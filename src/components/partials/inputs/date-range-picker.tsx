...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const selectedBg = isDarkMode ? 'bg-blue-700' : 'bg-blue-100';
    ...
    <div className="relative mb-6" ref={wrapperRef}>
      ...
      <div className="relative">
        <div
          ...
          className={`flex items-center justify-between ${bgColor} ${textColor} border ${borderColor} text-[11px] tracking-widest rounded-lg px-3 py-2.5 cursor-pointer ${
            disabled ? 'opacity-50 cursor-not-allowed' : ''
          } ${error ? 'border-red-600' : ''}`}
        >
          <span className={!value ? (isDarkMode ? 'text-gray-500' : 'text-gray-400') : ''}>{value || placeholder}</span>
      ...
      {showDateRangePicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`flex flex-col lg:flex-row ${modalBg} shadow-lg rounded-xl z-50 max-w-full max-h-[95vh] overflow-auto`}>
            {/* Left Sidebar */}
            <div className={`${borderColor} lg:border-r w-full lg:w-32 flex-shrink-0`}>
                  ...
                  {months.map((month, index) => (
                    <li key={index}>
                      <button
                        ...
                        className={`px-6 py-1.5 w-full leading-5 ${hoverBg} text-center ${textColor} ${
              ...
              {showYearsRange && !showMonthsRange1 && !showMonthsRange2 && (
                <div className="flex flex-col py-2">
                  <button
                    ...
                    className={`flex items-center justify-center p-2 ${hoverBg}`}
                  >
                    <svg className={`w-5 h-5 ${textColor} stroke-current`} fill="none">
              ...
              <div className="flex justify-center items-center mb-4">
                <div className="flex items-center justify-between w-full max-w-xs">
                  <button
                    ...
                    className={`flex items-center justify-center p-1.5 rounded-xl ${hoverBg}`}
                  >
                    <svg className={`w-5 h-5 ${textColor} stroke-current`} fill="none">
                  ...
                  <button
                    ...
                    className={`px-4 py-1 flex-1 mx-2 leading-5 ${hoverBg} text-center ${textColor} font-semibold text-xs`}
                  >
                    {currentYear}
                  </button>
                  <button
                ...
                <div className="flex flex-col">
                  <div className="flex items-center justify-between w-full mb-3">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 flex-1 mx-2 leading-5 ${hoverBg} text-center ${textColor} font-semibold text-xs`}
                    >
                  ...
                  <div className={`grid grid-cols-7 gap-1 text-[10px] tracking-widest text-center ${textColor} mb-2`}>
                  ...
                  <div className={`grid grid-cols-7 gap-1 text-[10px] tracking-widest text-center ${textColor}`}>
                    {paddedDays1.map((day, index) => (
                      <span
                        key={index}
                        onClick={() => day && selectDate(day, currentMonth1)}
                        className={`flex items-center justify-center w-8 h-8 rounded-lg ${
                          day ? 'cursor-pointer ' + hoverBg : ''
                        } ${isDateInRange(day || 0, currentMonth1) ? selectedBg : ''}`}
                      >
                        {day || ''}
                      </span>
                    ))}
                  ...
                  <div className={`grid grid-cols-7 gap-1 text-[10px] tracking-widest text-center ${textColor}`}>
                    {paddedDays2.map((day, index) => (
                      <span
                        ...
                        className={`flex items-center justify-center w-8 h-8 rounded-lg ${
                          day ? 'cursor-pointer ' + hoverBg : ''
                        } ${isDateInRange(day || 0, currentMonth2) ? selectedBg : ''}`}
                      >
                        {day || ''}
                      </span>
                    ))}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between mt-4 pt-4 border-t gap-3">
                <div className="flex items-center gap-2 text-[10px]">
                  <input
                    ...
                    className={`w-26 px-2 py-1.5 ${bgColor} ${textColor} rounded-lg border ${borderColor}`}
                    placeholder="Start Date"
                    disabled
                  />
                  <svg className={`w-6 h-6 ${textColor} stroke-current`} fill="none">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className={`px-3 py-1.5 text-[10px] rounded-lg ${bgColor} ${textColor} border ${borderColor} ${hoverBg}`}
                    onClick={() => setShowDateRangePicker(false)}
                  >
                    Cancel
                  </button>
                  <button
                    className={`px-3 py-1.5 text-[10px] text-white ${buttonBg} rounded-lg`}
